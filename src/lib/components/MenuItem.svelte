<script>
	...
	const baseClasses = isExpanded
		...
		: 'flex items-center justify-center p-2 w-full text-gray-800 rounded-md transition-colors hover:bg-black/10 active:bg-black/20 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 touch-manipulation'
</script>
...
{#if item.href}
	...
	>
		...
		{/if}
		<span class="flex-1 {isExpanded ? '' : 'sr-only'}">{item.label}</span>
	</a>
{:else if item.action}
	<!-- Render as button for actions -->
	<button type="button" class={baseClasses} {onclick} aria-label={item.label}>
		{#if isEmojiIcon(item.icon)}
			<span class={isExpanded ? 'text-2xl' : 'text-xl'} aria-hidden="true">
				{#if item.icon === 'whiskey'}
					🥃
